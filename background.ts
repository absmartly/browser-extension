    ...
    ;(async () => {
      try {
        ...
        if (tabs[0]?.id) {
          ...
          debugLog('Forwarding visual editor command to content script on tab:', tabId, 'URL:', tabUrl)
          ...
          try {
            ...
            sendResponse({ success: true, message: 'Visual editor started', response })

            /* COMMENTED OUT OLD INJECTION CODE - Now handled by content script

            // ALSO inject the actual functionality (the content script only has UI)
            // This is needed because VisualEditor class only has stub implementations
            // Get the logo URL to pass to the visual editor
            const logoUrl = chrome.runtime.getURL('assets/absmartly-logo-white.svg')

            // Now inject the visual editor using a lightweight function that loads the bundled script
            const injectionResult = await chrome.scripting.executeScript({
                target: { tabId: tabId },
                func: async function(variantName, experimentName, logoUrl, initialChanges) {
                  console.log('[ABSmartly] Loading visual editor from bundled script...')
                  ...
                  try {
                    // Fetch the bundled visual editor script
                    const bundledScriptUrl = chrome.runtime.getURL('src/injected/build/visual-editor-injection.js')
                    const response = await fetch(bundledScriptUrl)
                    const scriptCode = await response.text()

                    // Create a function wrapper that provides the parameters
                    const wrappedCode = `
                      (function(variantName, experimentName, logoUrl, initialChanges) {
                        ${scriptCode}
                    ...
                    console.log('[ABSmartly] Bundled visual editor loaded successfully')
                    return result || { success: true }

                  } catch (error) {
                    console.error('[ABSmartly] Error loading bundled visual editor:', error)

                    // Fallback to minimal visual editor
                    console.log('[ABSmartly] Using fallback minimal visual editor')
                    ...
                    `
                    banner.textContent = `Visual Editor Active (Fallback Mode) - ${experimentName} → ${variantName}`
            ...
            */
            // END OF COMMENTED OUT INJECTION CODE

          } catch (error) {
            debugError('Failed to forward message to content script:', error)
            sendResponse({ success: false, error: error.message || 'Failed to forward message to content script' })
          }
        } else {

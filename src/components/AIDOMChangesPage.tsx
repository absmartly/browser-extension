...
} from "~src/utils/ai-conversation-storage"
...
const generateMountId = () =>
  `mount-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`
...
export const AIDOMChangesPage = React.memo(function AIDOMChangesPage({
  ...
  const [latestDomChanges, setLatestDomChanges] =
    useState<DOMChange[]>(currentChanges)
  ...
  useEffect(() => {
    ...
    if (onPreviewWithChanges) {
      debugLog(
        "[AIDOMChangesPage] Auto-enabling preview on mount with existing changes:",
        latestDomChanges.length
      )
  ...
  const handleGenerate = async () => {
    debugLog(
      "[AIDOMChangesPage] handleGenerate called, prompt:",
      prompt,
      "images:",
    ...
    const llmImages =
      attachedImages.length > 0
        ? await compressImagesForLLM(attachedImages)
        : undefined
    ...
    try {
      ...
      try {
        await saveCurrentConversation(
          newHistory,
          result.session || conversationSession!
        )
      } catch (storageError) {
        debugWarn(
          "[AIDOMChangesPage] Failed to save conversation:",
          storageError
        )
      ...
      if (result.domChanges && result.domChanges.length > 0) {
        ...
        console.log(
          "[AI Generate] AIDOMChangesPage final changes count:",
          finalChanges.length
        )
        ...
        console.log(
          "[AI Persist] Storing AI changes for",
          variantName,
          "count:",
          finalChanges.length
        )
        ...
        storageResults.forEach((result) => {
          if (result.status === "rejected") {
            debugWarn(
              "[AIDOMChangesPage] Failed to persist AI changes:",
              result.reason
            )
          }
        })
        try {
          ...
          if (Array.isArray(savedVariants)) {
            const targetIndex = savedVariants.findIndex(
              (v) => v?.name === variantName
            )
            if (targetIndex !== -1) {
              const currentConfig = savedVariants[targetIndex]?.config || {}
              const domFieldName =
                Object.keys(currentConfig).find((k) =>
                  k.includes("dom_changes")
                ) || "__dom_changes"
          ...
          debugWarn(
            "[AIDOMChangesPage] Failed to update stored variants with AI changes:",
            storageError
          )
        }
        ...
        if (!previewEnabledOnce) {
          debugLog(
            "[AIDOMChangesPage] First message with DOM changes - enabling preview directly"
          )
          if (onPreviewWithChanges) {
          ...
          debugLog(
            "[AIDOMChangesPage] Subsequent message - clearing old changes then applying new ones"
          )
      ...
      debugLog(
        "[AIDOMChangesPage] Clearing prompt and images, then setting loading to false"
      )
      setPrompt("")
      setAttachedImages([])
      setLoading(false)
  ...
  const handlePreviewToggle = () => {
    ...
    if (newState) {
      ...
      debugLog(
        "[AIDOMChangesPage] Preview toggled ON - reapplying changes:",
        latestDomChanges.length
      )
      if (onPreviewWithChanges) {
        onPreviewWithChanges(true, latestDomChanges)
  ...
  const handleViewChanges = useCallback(
    (changes: DOMChange[], response: string, timestamp: number) => {
      setViewerModal({ changes, response, timestamp })
    },
    []
  )

  const handleRestoreChangesFromHistory = useCallback(
    (changes: DOMChange[]) => {
  ...
  const handlePreviewWithChanges = useCallback(
    (enabled: boolean, changes: DOMChange[]) => {
      if (onPreviewWithChanges) {
  ...
  const handleRestoreChangesCallback = useCallback(
    (changes: DOMChange[]) => {
      if (onRestoreChanges) {
        ...
        <Header
          title={
            <div className="flex items-center gap-2">
              ...
              <div>
                <h2
                  id="ai-dom-generator-heading"
                  className="text-lg font-semibold text-gray-900">
                  Vibe Studio
                </h2>
                <p id="ai-variant-label" className="text-xs text-gray-600">
                  Variant: {variantName}
                </p>
              </div>
            </div>
              ...
              <div>
                <button
                  ...
                  {isLoadingHistory ? (
                    ...
                    <ClockIcon
                      className={`h-5 w-5 ${conversationList.length === 0 ? "text-gray-300" : "text-gray-600"}`}
                    />
                  )}
                </button>

...
import {
  getExperimentStateBadgeVariant,
  getExperimentStateLabel
} from "~src/utils/experiment-state"
import { getConfig, localAreaStorage as storage } from "~src/utils/storage"
...
export function ExperimentDetail({
  ...
  debugLog(
    "🔍 ExperimentDetail render start - experiment.variants:",
    experiment?.variants
  )
  debugLog("🔍 ExperimentDetail render start - loading:", loading)
  debugLog(
    "🔍 ExperimentDetail props - unitTypes:",
    unitTypes?.length,
    "owners:",
    owners?.length,
    "tags:",
  ...
  const { save, saving } = useExperimentSave({
    experiment,
    domFieldName,
    onError
  })
  const [metadata, setMetadata] = useState({
    ...
    application_ids:
      experiment.applications?.map((a) => a.application_id || a.id) || [],
  ...
  debugLog(
    "🔍 ExperimentDetail state - variants length:",
    experiment?.variants?.length
  )
  debugLog(
    "🔍 ExperimentDetail state - should show variants section:",
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      try {
        ...
        debugLog(
          "🧹 Cleared all DOM changes storage (session + local) after save for experiment",
          experiment.id
        )
      } catch (error) {
  ...
  const handleBack = () => {
    ...
    if (hasUnsavedChanges) {
      if (
        ...
        debugLog(
          "🧹 User chose to discard changes for experiment",
          experiment.id
        )
        clearAllExperimentStorage(experiment.id)
          .then(() => {
            debugLog(
              "🧹 Cleared all DOM changes storage (session + local) when discarding for experiment",
              experiment.id
            )
            cleanup()
            onBack()
      ...
      {editingName ? (
        <div>
          ...
          {displayName !== experiment.name && (
            <p
              className="text-sm text-gray-500 mt-1 overflow-hidden text-ellipsis whitespace-nowrap cursor-help"
  ...
  const actions = (
    <>{!editingName && <ExperimentActions experimentId={experiment.id} />}</>
  )

  return (
    <div className="p-4">
      <Header title={titleContent} onBack={handleBack} />
      ...
      <div className="mb-4 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <Badge
            variant={getExperimentStateBadgeVariant(
          ...
          <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg flex items-start gap-2">
            ...
            <div className="flex-1">
              <p className="text-sm text-yellow-800">
                ...
                Changes cannot be saved while the experiment is active. Restart
                the experiment to make changes.
              </p>
            </div>
          </div>
        )}

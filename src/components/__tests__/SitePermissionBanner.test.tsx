...
const makePermission = (overrides: Partial<ActiveSitePermissionState> = {}): ActiveSitePermissionState => ({
...
describe('SitePermissionBanner', () => {
  ...
  it('renders nothing when permission is already granted', () => {
    const { container } = render(
      <SitePermissionBanner permission={makePermission({ hasPermission: true })} />
    )
    expect(container).toBeEmptyDOMElement()
  })
  ...
  it('renders nothing on schemes where host permission does not apply (chrome://, etc.)', () => {
    const { container } = render(
      <SitePermissionBanner permission={makePermission({ host: null, originPattern: null })} />
    )
    expect(container).toBeEmptyDOMElement()
  })
  ...
  it('renders the permission prompt with the active host when permission is missing', () => {
    ...
    expect(screen.getByRole('alert')).toHaveAttribute('id', 'site-permission-banner')
  ...
  it('invokes requestPermission when the Grant access button is clicked', async () => {
    ...
    render(
      <SitePermissionBanner permission={makePermission({ requestPermission })} />
    )
  ...
  it('surfaces a denial message when the user rejects the native prompt', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /Grant access/i }))
    await waitFor(() => expect(screen.getByText(/Permission was not granted/i)).toBeInTheDocument())
  })
})

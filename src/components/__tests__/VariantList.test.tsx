...
jest.mock("~src/hooks/useVariantConfig", () => ({
  ...
  setDOMChangesInConfig: jest.fn(
    (config, domChanges, domFieldName = "__dom_changes") => {
  ...
  getVariablesForDisplay: jest.fn(
    (config, domFieldName, fieldsToExclude = ["__inject_html"]) => {
...
describe("VariantList", () => {
  ...
  describe("Add/Delete Variants", () => {
    ...
    it("should delete variant", async () => {
      ...
      const lastCall =
        onVariantsChange.mock.calls[onVariantsChange.mock.calls.length - 1]
  ...
  describe("DOM Changes Management", () => {
    ...
    it("should filter disabled changes from preview", async () => {
      ...
      render(
        <VariantList {...defaultProps} initialVariants={variantsWithDisabled} />
      )
  ...
  describe("Storage Integration", () => {
    it("should load saved variants from storage on mount", async () => {
      ...
      ;(storage.localAreaStorage.get as jest.Mock).mockResolvedValueOnce(
        savedVariants
      )
      ...
      await waitFor(() => {
        expect(storage.localAreaStorage.get).toHaveBeenCalledWith(
    ...
    it("should handle storage errors gracefully", async () => {
      ;(storage.localAreaStorage.get as jest.Mock).mockRejectedValueOnce(
  ...
  describe("Visual Editor Integration", () => {
    ...
    it("should prevent opening VE when another is active", async () => {
      ...
      const veButtons = screen.queryAllByRole("button", {
        name: /visual editor/i
      })

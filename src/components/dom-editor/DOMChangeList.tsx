...
export function DOMChangeList({
  ...
  const getChangeDescription = (change: DOMChange): React.ReactNode => {
    switch (change.type) {
      case "text":
        return (
          <span className="text-gray-600">
            <span className="text-gray-500">Set text to:</span>{" "}
            <span className="font-medium text-gray-800">"{change.value}"</span>
          </span>
        )
      case "style":
        ...
        return (
          <div className="flex flex-wrap gap-1.5">
            {styles.map(([key, value], i) => (
              <span
                key={i}
                className="inline-flex items-center px-1.5 py-0.5 rounded bg-blue-50 text-xs">
      ...
      case "styleRules":
        ...
        const stateCount = Object.entries(styleRulesChange.states || {}).filter(
          ([_, styles]) => styles && Object.keys(styles).length > 0
        ).length
        return (
          <div className="flex flex-wrap gap-1.5">
            {Object.entries(styleRulesChange.states || {}).map(
              ([state, styles]) => {
                ...
                return (
                  <span
                    key={state}
                    className="inline-flex items-center px-1.5 py-0.5 rounded bg-purple-50 text-xs">
      ...
      case "class":
        if (
          (!change.add || change.add.length === 0) &&
          (!change.remove || change.remove.length === 0)
        ) {
          return (
        ...
        return (
          <div className="flex flex-wrap gap-1.5">
            {change.add?.map((cls, i) => (
              <span
                key={`add-${i}`}
                className="inline-flex items-center px-1.5 py-0.5 rounded bg-green-50 text-xs">
            ...
            {change.remove?.map((cls, i) => (
              <span
                key={`remove-${i}`}
                className="inline-flex items-center px-1.5 py-0.5 rounded bg-red-50 text-xs">
  ...
  const renderedChanges = useMemo(() => {
    return changes.map((change, index) => {
      ...
      const diagnostic =
        change.type === "javascript" && changeDiagnostics && getChangeKey
          ...
          <div className="p-3">
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-sm">{getChangeDescription(change)}</div>
                {diagnostic && (
                  <div
              ...
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation()
                  navigator.clipboard
                    .writeText(JSON.stringify([change], null, 2))
                    .then(() => {
  ...
  for (let i = 0; i < renderedChanges.length; i++) {
    if (editingIndex === i && editorSlot) {
      items.push(
        <React.Fragment key={`editor-${i}`}>{editorSlot}</React.Fragment>
      )
    } else {

  ...
  const getDOMChangesCount = (): number => {
    ...
    experiment.variants.forEach((variant) => {
      try {
        ...
        debugWarn(
          `[ExperimentListItem] Failed to parse variant config for "${variant.name}":`,
          error
        )
      }
    })
      ...
      <div
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-2 flex items-center gap-3 flex-wrap">
            <span
              className={`inline-flex items-center h-[26px] px-3 text-xs font-medium rounded-full ${getExperimentStateClasses(status)}`}>
            ...
            {domChangesCount > 0 && (
              <div className="relative group">
                ...
                <div className="absolute bottom-full mb-2 left-1/2 transform -translate-x-1/2 px-2 py-1 text-xs text-white bg-gray-900 rounded opacity-0 pointer-events-none group-hover:opacity-100 transition-opacity whitespace-nowrap z-50">
                  {domChangesCount} DOM{" "}
                  {domChangesCount === 1 ? "change" : "changes"}
          ...
          <div className="relative group">
            <button
              onClick={(e) => {
                e.stopPropagation()
                const endpoint =
                  localStorage.getItem("absmartly-endpoint") || ""

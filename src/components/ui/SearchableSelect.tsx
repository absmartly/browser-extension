...
export function SearchableSelect(props: SearchableSelectProps) {
  ...
  const selectedOptions =
    mode === "single"
      ? props.selectedId !== null && props.selectedId !== undefined
        ? options.filter((opt) => String(opt.id) === String(props.selectedId))
        : []
  ...
  const getInitials = (name: string) => {
    return name
      ...
      .filter(
        (part, i, arr) => part.length > 0 && (i === 0 || i === arr.length - 1)
      )
  ...
  const defaultRenderOption = (
    ...
    const initials =
      option.type === "team"
        ? (
            option.initials || (option.display_name || option.name).charAt(0)
          ).toUpperCase()
    ...
    const colorClass = isHexColor
      ? ""
      : option.color || getDefaultColor(options.indexOf(option))

    const shapeClass = option.type === "team" ? "rounded-md" : "rounded-full"
    const borderClass =
      option.type === "team" ? "border-[0.5px] border-border" : ""
              ...
              mode === "single" ? (
                renderSelectedOption ? (
                  renderSelectedOption(
                    selectedOptions[0],
                    !required && !disabled
                      ? (e) => handleRemoveOption(selectedOptions[0].id, e)
                      : undefined
                  )
                ) : (
                ...
                selectedOptions.map((opt) =>
                  renderSelectedOption
                    ? renderSelectedOption(opt, (e) =>
                        handleRemoveOption(opt.id, e)
                      )
                    : defaultRenderSelectedOption(opt, (e) =>
                        handleRemoveOption(opt.id, e)
                      )
                )
              )
          ...
          <div className="max-h-60 overflow-y-auto">
            {filteredOptions.length > 0 ? (
              filteredOptions.map((opt) =>
                renderOption
                  ? renderOption(opt, isSelected(opt.id), () =>
                      handleSelectOption(opt.id)
                    )
                  : defaultRenderOption(opt, isSelected(opt.id), () =>
                      handleSelectOption(opt.id)
                    )
              )
            ) : (

...
const renderInit = (overrides: HookDeps = {}) => {
  const deps = {
    ...
    loadExperiments:
      overrides.loadExperiments ?? jest.fn().mockResolvedValue(undefined),
    loadEditorResources:
      overrides.loadEditorResources ?? jest.fn().mockResolvedValue(undefined),
    loadFavorites:
      overrides.loadFavorites ?? jest.fn().mockResolvedValue(undefined),
    getApplications:
      overrides.getApplications ?? jest.fn().mockResolvedValue([]),
  ...
  describe("create/edit view editor resources", () => {
    ...
    it("does not reload editor resources if unit types are already populated", async () => {
      const { deps } = renderInit({
        ...
        hasInitialized: true,
        unitTypes: [
          { id: 1, name: "user", unit_type_id: 1 } as unknown as UnitType
        ]
      })

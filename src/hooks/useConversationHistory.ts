...
import {
  migrateConversation,
  needsMigration
} from "~src/utils/ai-conversation-migration"
import {
  deleteConversation,
  getConversationList,
  loadConversation,
...
export function useConversationHistory(variantName: string) {
  ...
  const initializeSessionHTML = useCallback(
    async (session: ConversationSession, conversationId: string) => {
      debugLog("[initializeSessionHTML] Starting background HTML capture...")

      try {
        const captureResult = await capturePageHTML()
        debugLog(
          "[initializeSessionHTML] HTML captured, length:",
          captureResult.html.length,
          "URL:",
          captureResult.url,
          "structure lines:",
          captureResult.domStructure?.split("\n").length
        )
        ...
        if (response.success && response.session) {
          ...
          debugLog(
            "[initializeSessionHTML] ✅ HTML context initialized successfully"
          )
        } else {
          throw new Error(
            (response.error as string) || "Failed to initialize session"
          )
        }
      } catch (error) {
  ...
  useEffect(() => {
    debugLog(
      "[useConversationHistory] ========== INITIALIZATION START =========="
    )
    ...
    ;(async () => {
      try {
        ...
        debugLog(
          "[useConversationHistory] Loaded conversation list:",
          list.length,
          "conversations for variant:",
          variantName
        )
        ...
        if (activeConv) {
          ...
          if (loaded) {
            ...
            debugLog(
              "[useConversationHistory] Loaded active conversation:",
              loaded.id
            )

            if (!loaded.conversationSession.htmlSent) {
              initializeSessionHTML(
                loaded.conversationSession,
                loaded.id
              ).catch((err) => {
                debugWarn(
                  "[useConversationHistory] HTML initialization failed (non-blocking):",
                  err
                )
              })
            }
        ...
        debugLog(
          "[useConversationHistory] Created new conversation:",
          newConvId
        )

        initializeSessionHTML(newSession, newConvId).catch((err) => {
          debugWarn(
            "[useConversationHistory] HTML initialization failed (non-blocking):",
            err
          )
        ...
        console.error(
          "[useConversationHistory] ❌ INITIALIZATION ERROR:",
          error
        )
        setError(
    ...
    if (chatHistory.length > 0 && currentConversationId) {
      const currentConv: StoredConversation = {
        ...
        firstUserMessage:
          chatHistory
            .find((m) => m.role === "user")
            ?.content.substring(0, 50) || "Conversation",
        isActive: false
      }
      ...
      try {
        ...
        debugWarn(
          "[useConversationHistory] Failed to save conversation before creating new chat:",
          storageError
        )
      }
    }
    ...
    async (conv: ConversationListItem) => {
      ...
      if (chatHistory.length > 0 && currentConversationId !== conv.id) {
        ...
        try {
          ...
          debugWarn(
            "[useConversationHistory] Failed to save conversation before switching:",
            storageError
          )
        }
      }
  ...
  const refreshHTML = useCallback(async () => {
    ...
    try {
      ...
      const captureResult = await capturePageHTML()
      debugLog(
        "[useConversationHistory] HTML captured, length:",
        captureResult.html.length,
        "URL:",
        captureResult.url,
        "structure lines:",
      ...
      if (response.success && response.session) {
        ...
        debugLog(
          "[useConversationHistory] ✅ HTML context refreshed successfully"
        )
        return true
      } else {
  ...
  const saveCurrentConversation = useCallback(
    async (messages: ChatMessage[], session: ConversationSession) => {
      ...
      try {
        ...
        debugWarn(
          "[useConversationHistory] Failed to save conversation:",
          storageError
        )
        throw storageError
      }

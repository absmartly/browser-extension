...
export function useEditorStateRestoration({
  ...
  useEffect(() => {
    ...
    storage
      ...
      .then(async (result) => {
        debugLog(
          "Checking for saved DOM Changes inline state, variantName:",
          variantName
        )
        ...
        if (result && result.variantName === variantName) {
          ...
          if (
            pickerResult &&
            pickerResult.variantName === variantName &&
            ...
            if (pickerResult.fieldId === "selector" && result.editingChange) {
              ...
              pickerResult.fieldId === "targetSelector" &&
              result.editingChange
            ) {
              setEditingChange({
                ...result.editingChange,
              ...
              pickerResult.fieldId === "observerRoot" &&
              result.editingChange
            ) {
              setEditingChange({
                ...result.editingChange,
                observerRoot: pickerResult.selector
              })
            }
          ...
          const dragDropResult =
            await storage.get<DragDropResult>("dragDropResult")
          ...
          if (dragDropResult && dragDropResult.variantName === variantName) {
            ...
            if (result.editingChange) {
              ...
              debugLog(
                "Updated editingChange with drag-drop result:",
                updatedChange
              )
              ...
              setTimeout(() => {
                debugLog(
                  "🔄 Force checking editingChange after setEditingChange:",
                  editingChange
                )
                setEditingChange(updatedChange)
              }, 100)
              ...
              debugWarn(
                "No editingChange found in restored state, cannot apply drag-drop result"
              )
            }
            ...
            debugLog(
              "Drag-drop result not applicable:",
              "variantName match:",
              dragDropResult?.variantName === variantName,
              "dragDropResult exists:",
              !!dragDropResult
            )
          }
        ...
        if (
          visualEditorResult &&
          visualEditorResult.variantName === variantName
        ) {
          debugLog("Found visual editor changes for this variant!")
          if (
            visualEditorResult.changes &&
            visualEditorResult.changes.length > 0
          ) {
            const merged = [...changes]
            for (const change of visualEditorResult.changes) {
              const existingIndex = merged.findIndex(
                (c) => c.type === change.type && c.selector === change.selector
              )
              if (existingIndex >= 0) {
        ...
        const aiDomChangesState = await storage.get<{
          variantName: string
          changes: DOMChange[]
          timestamp?: number
        }>("aiDomChangesState")
        const localAiDomChangesState = await localAreaStorage.get<{
          variantName: string
          changes: DOMChange[]
          timestamp?: number
        }>("aiDomChangesState")
        ...
        if (effectiveAIState && effectiveAIState.variantName === variantName) {
          if (
            effectiveAIState.changes &&
            effectiveAIState.changes.length > 0 &&
            changes.length === 0
          ) {
            debugLog(
              "Restoring AI-generated DOM changes from storage (current changes empty)"
            )
            onChange(effectiveAIState.changes)
          } else {
            debugLog(
              "Skipping AI state restoration - changes already present:",
              changes.length
            )
          }
          await Promise.all([
  ...
  useEffect(() => {
    const handleElementSelected = (
      ...
      debugLog(
        "[useEditorStateRestoration] RECEIVED ELEMENT_SELECTED, pickingForField:",
        pickingForField
      )
      ...
      if (
        ...
        const storage = sessionStorage
        debugLog(
          "[useEditorStateRestoration] Using pickingForField as fieldId:",
          pickingForField
        )
        ...
        debugLog(
          "[useEditorStateRestoration] Stored to sessionStorage:",
          pickerResult
        )
        ...
        if (pickingForField && editingChange) {
          debugLog(
            "[useEditorStateRestoration] Applying picker result immediately to field:",
            pickingForField
          )
  ...
  useEffect(() => {
    ...
    const handleDragDropComplete = async (message: ChromeMessage) => {
      if (
        message.type === "DRAG_DROP_COMPLETE" &&
        "selector" in message &&
        "targetSelector" in message &&
        "position" in message
      ) {
        debugLog(
          "📡 Received drag-drop message in DOMChangesInlineEditor:",
          message
        )

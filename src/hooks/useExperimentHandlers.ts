...
export function useExperimentHandlers({
  ...
  const handleExperimentClick = useCallback(
    async (experiment: Experiment) => {
      try {
        ...
        console.error(
          "[Storage] Storage clear failed - extension may have stale data:",
          {
            ...
            error:
              storageErr instanceof Error
                ...
                : String(storageErr),
            suggestion:
              "Extension may show outdated information. Consider reloading the extension."
          }
        )
        onError(
          "Failed to clear storage. Extension may show outdated information."
        )
      }
  ...
  const handleCreateExperiment = useCallback(async () => {
    try {
      ...
      debugError(
        "[Storage] Failed to clear storage for new experiment:",
        storageErr
      )
  ...
  const handleCreateFromTemplate = useCallback(
    async (templateId: number) => {
      try {
        ...
        debugError(
          "[Storage] Failed to clear storage for template:",
          storageErr
        )
        console.error("[Storage] Storage clear failed:", storageErr)
  ...
  const handleEditExperiment = useCallback(
    async (experiment: Experiment) => {
      try {
        ...
        console.error("[Storage] Storage clear failed:", storageErr)
        onError(
          "Failed to clear storage. Edited experiment may have stale data."
        )
      }
      setSelectedExperiment(experiment)
      setView("edit")
  ...
  const handleSaveExperiment = useCallback(
    async (experiment: Partial<Experiment>) => {
      try {

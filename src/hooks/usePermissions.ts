...
export function usePermissions(config: ABsmartlyConfig | null) {
  ...
  const requestPermissionsIfNeeded = useCallback(
    async (forceRequest = false): Promise<boolean> => {
      debugLog(
        "[ExtensionUI] requestPermissionsIfNeeded called, forceRequest:",
        forceRequest
      )

      if (config?.authMethod !== "jwt") {
        debugLog(
          "[ExtensionUI] Not using JWT auth, skipping permission request"
        )
        return false
      }
      ...
      try {
        ...
        debugLog(
          "[ExtensionUI] Permission status - cookies:",
          hasCookies,
          "host:",
          hasHost
        )
  ...
  const handleGrantPermissions = async (
    ...
    debugLog(
      "[ExtensionUI] handleGrantPermissions called - requesting permissions"
    )

    try {
      const currentCookies = await chrome.permissions.contains({
        permissions: ["cookies"]
      })
      const currentHost = await chrome.permissions.contains({
        origins: ["https://*.absmartly.com/*"]
      })
      debugLog(
        "[ExtensionUI] Current permissions before request - cookies:",
        currentCookies,
        "host:",
        currentHost
      )
      ...
      const finalCookies = await chrome.permissions.contains({
        permissions: ["cookies"]
      })
      const finalHost = await chrome.permissions.contains({
        origins: ["https://*.absmartly.com/*"]
      })
      debugLog(
        "[ExtensionUI] Final permissions after request - cookies:",
        finalCookies,
        "host:",
        finalHost
      )
      ...
      if (cookiesGranted && hostGranted) {
        ...
        if (currentCookies && currentHost) {
          debugLog(
            "[ExtensionUI] ⚠️ Permissions were already granted - JWT cookie might be missing or expired"
          )
          onError(
            "You have permissions but authentication failed. Please log in to ABsmartly in your browser first."
          )
        } else {
          onSuccess()
        }
        ...
        console.error(
          "[ExtensionUI] ❌ User denied some permissions - cookies:",
          cookiesGranted,
          "host:",
          hostGranted
        )
        setNeedsPermissions(false)
        onError(
          "Cookie and host permissions are required for JWT authentication."
        )
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (config?.authMethod === "jwt") {
      debugLog(
        "[ExtensionUI] Checking permissions on mount (JWT auth detected)"
      )
      chrome.permissions
        .contains({
          permissions: ["cookies"],
        ...
        .then((hasPermission) => {
          if (!hasPermission) {
            debugWarn(
              "[ExtensionUI] ⚠️ Missing cookie permissions! Will request on first API call."
            )
          } else {

...
export function useViewNavigation() {
  ...
  const handleNavigateToAI = useCallback(
    (
      ...
      debugLog(
        "[useViewNavigation] handleNavigateToAI called for variant:",
        variantName
      )
      debugLog("[useViewNavigation] Current view:", view)
      if (view === "ai-dom-changes" || isNavigatingToAIRef.current) {
        debugLog(
          "[useViewNavigation] Already navigating or in AI view, skipping"
        )
        return
      }
      isNavigatingToAIRef.current = true
      debugLog(
        "[useViewNavigation] Setting aiDomContext and navigating to ai-dom-changes view"
      )
      ...
      debugLog(
        "[useViewNavigation] Navigation complete. New view should be: ai-dom-changes"
      )
    },
    [view]
  )
  ...
  useEffect(() => {
    ...
    const state = {
      ...
      aiVariantName:
        view === "ai-dom-changes" ? aiDomContext?.variantName : null
    }

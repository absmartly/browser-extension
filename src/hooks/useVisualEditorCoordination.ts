...
export function useVisualEditorCoordination({
  ...
  useEffect(() => {
    const handleVisualEditorChanges = (
      ...
      if (
        message.type === "VISUAL_EDITOR_CHANGES" &&
        "variantName" in message &&
        message.variantName === variantName
      ) {
        ;(async () => {
          debugLog(
            "📡 Visual editor changes received:",
            "changes" in message ? message.changes : []
          )
        ...
        message.type === "VISUAL_EDITOR_CHANGES_COMPLETE" &&
        "variantName" in message &&
        message.variantName === variantName
      ) {
        ;(async () => {
          ...
          if (
            ...
            debugLog(
              "📝 Merging final visual editor changes with existing changes"
            )
    ...
    debugLog(
      "[DOMChangesInlineEditor] Setting up message listeners for variant:",
      variantName
    )
    ...
    const handleWindowMessage = (event: MessageEvent) => {
      ...
      if (!isFileProtocol && event.origin !== window.location.origin) {
        debugLog(
          "[Security] Ignoring Visual Editor message from origin:",
          event.origin
        )
        return
      }
      ...
      if (event.data?.source === "absmartly-visual-editor") {
        handleVisualEditorChanges(
          event.data as ChromeMessage,
    ...
    return () => {
      debugLog(
        "[DOMChangesInlineEditor] Cleaning up message listeners for variant:",
        variantName
      )
  ...
  const handleLaunchVisualEditor = useCallback(async () => {
    try {
      ...
      debugLog(
        "[DOMChangesInlineEditor] 🎨 Launch requested for variant:",
        variantName
      )
      debugLog(
        "[DOMChangesInlineEditor] 🎨 activeVEVariant state:",
        activeVEVariant
      )
      debugLog(
        "[DOMChangesInlineEditor] 🎨 isLaunchingVisualEditor flag:",
        isLaunchingVisualEditor
      )

      if (activeVEVariant) {
        if (activeVEVariant === variantName) {
          debugLog(
            "[DOMChangesInlineEditor] Visual Editor already active for this variant, ignoring click"
          )
          return
        } else {
          alert(
            `Visual Editor is already active for variant "${activeVEVariant}". Please close it first.`
          )
          return
        }
      }
      ...
      if (isLaunchingVisualEditor) {
        debugLog(
          "⏭️ Visual Editor already launching, skipping duplicate launch for:",
          variantName
        )
        return
      }
      ...
      if (!tabs || tabs.length === 0) {
        debugLog(
          "[DOMChanges] ⚠️ No active tab found, querying all tabs as fallback"
        )
      ...
      try {
        debugLog(
          "[DOMChangesInlineEditor] 🎨 Launching Visual Editor for variant:",
          variantName
        )
        debugLog(
          "[DOMChangesInlineEditor] Current preview state:",
          previewEnabled
        )
        ...
        debugLog(
          "[DOMChangesInlineEditor] onVEStart function:",
          onVEStart.toString()
        )
        onVEStart()
        ...
        if (tabs[0]?.id) {
          try {
            ...
            debugLog(
              "[DOMChangesInlineEditor] ✅ SET_VISUAL_EDITOR_STARTING sent successfully"
            )
          } catch (e) {
            const error = e as Error
            console.error(
              "[DOMChangesInlineEditor] ⚠️ SET_VISUAL_EDITOR_STARTING failed (content script may not be ready yet):",
              error?.message
            )
          }
        }
        ...
        debugLog(
          "[DOMChanges] About to send START_VISUAL_EDITOR, tabs[0]?.id:",
          tabs[0]?.id
        )
        if (tabs[0]?.id) {
          ...
          try {
            debugLog(
              "[DOMChanges] 📤 Sending START_VISUAL_EDITOR to content script"
            )
            debugLog("[DOMChanges] Variant:", variantName)
            ...
            debugLog(
              "[DOMChanges] 🔔 RESPONSE from content script:",
              JSON.stringify(response)
            )

            if (response?.error) {
              console.error(
                "[DOMChanges] ❌ Error from content script:",
                response.error
              )
            ...
            const error = e as Error
            console.error(
              "[DOMChanges] ❌ Exception while sending to content:",
              error?.message
            )
      ...
      console.error(
        "[DOMChanges] ❌ FATAL ERROR in handleLaunchVisualEditor:",
        error
      )

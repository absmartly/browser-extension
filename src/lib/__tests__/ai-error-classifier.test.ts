...
describe("AI Error Classifier", () => {
  describe("classifyAIError", () => {
    it("should classify timeout errors", () => {
      ...
      expect(classified.userAction).toBe(
    ...
    it("should classify model not found errors", () => {
      ...
      expect(classified.userAction).toBe(
        "Try a different model or check model availability"
      )
    })
  ...
  describe("formatClassifiedError", () => {
    it("should format error message with user action", () => {
      ...
      expect(formatted).toBe(
        "AI service timed out. Try a simpler request or try again later"
      )
    })
  ...
  describe("real-world error messages", () => {
    it("should handle OpenAI rate limit error", () => {
      const error = new Error(
        "Rate limit reached for gpt-4-turbo in organization org-xxx on tokens per min."
      )
      const classified = classifyAIError(error)
    ...
    it("should handle Gemini model error", () => {
      const error = new Error(
        "models/gemini-pro-xxx not found. Invalid model name."
      )
      const classified = classifyAIError(error)

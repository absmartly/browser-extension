...
describe("AI Error Handling - End-to-End Flow", () => {
  ...
  it("should propagate invalid model error to UI with clear message", async () => {
    ...
    const invalidModelError =
      'Invalid model: "invalid-model" is not supported. Available models: sonnet, opus, haiku'
  ...
  it("should handle API authentication errors clearly", async () => {
    ...
    const authError =
      "Authentication failed: Invalid API key or subscription expired"
  ...
  it("should preserve all error context when displaying to user", async () => {
    ...
    const complexError =
      'Request failed: Model "moonshot/kimi-k2.5" requires pro subscription. Current plan: free. Upgrade at https://example.com/upgrade'

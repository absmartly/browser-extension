...
describe("BridgeProvider", () => {
  ...
  describe("generate", () => {
    it("should connect to bridge before generating", async () => {
      ...
      mockBridgeClient.streamResponses.mockImplementation(
        (conversationId, onEvent, onError) => {
          // Fire events after sendMessage timeout (100ms) plus buffer
          setTimeout(() => {
            onEvent({
              type: "tool_use",
              data: { domChanges: [], response: "Test", action: "none" }
            })
    ...
    it("should handle invalid model error with user-friendly message", async () => {
      ...
      const invalidModelError =
        "Model not found: invalid-model-name is not a valid model. Available models: sonnet, opus, haiku"
    ...
    it("should handle API error with parseable error object", async () => {
      ...
      try {
        ...
        console.log(
          "API error message that would be shown to user:",
          errorMessage
        )
      }
    })
    ...
    it("should format error messages for user display without double prefixes", async () => {
      ...
      try {
        ...
        expect(errorMessage).toBe(
          'AI CLI Bridge error: Invalid model "gpt-5" - model not found'
        )

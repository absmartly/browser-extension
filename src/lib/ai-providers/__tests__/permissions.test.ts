import { hasProviderPermissions, ensureProviderPermissions } from '../permissions'
...
const mockedGetProviderOrigins = getProviderOrigins as jest.MockedFunction<typeof getProviderOrigins>
...
describe('permissions', () => {
  ...
  describe('hasProviderPermissions', () => {
    ...
    it('should return true when chrome.permissions.contains returns true', async () => {
      ...
      expect(result).toBe(true)
      expect(mockContains).toHaveBeenCalledWith({ origins: ['https://api.anthropic.com/*'] })
    })

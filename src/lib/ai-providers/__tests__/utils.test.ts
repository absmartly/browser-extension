...
jest.mock("~src/prompts/ai-dom-generation-system-prompt", () => ({
  AI_DOM_GENERATION_SYSTEM_PROMPT:
    "Default system prompt {{CHUNK_RETRIEVAL_DOCUMENTATION}} for AI DOM generation"
}))
...
describe("AI Provider Utils", () => {
  ...
  describe("compressHtml", () => {
    ...
    it("should return empty string for undefined input", () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        "[Compress] ❌ HTML is undefined or empty!"
      )
    })
    ...
    it("should remove Plasmo loading div", () => {
      const html =
        '<div><div id="__plasmo-loading__"><div>Loading...</div></div><p>Content</p></div>'
    ...
    it("should remove elements with data-plasmo attributes", () => {
      const html =
        '<div data-plasmo-something="true">Extension</div><p>Content</p>'
      const result = compressHtml(html)
    ...
    it("should remove absmartly sidebar iframe", () => {
      const html =
        '<iframe id="absmartly-sidebar-iframe">Sidebar</iframe><p>Content</p>'
      const result = compressHtml(html)
    ...
    it("should remove chrome-extension iframes", () => {
      const html =
        '<iframe src="chrome-extension://abc123">Extension</iframe><p>Content</p>'
      const result = compressHtml(html)
    ...
    it("should remove absmartly preview header", () => {
      const html =
        '<div id="absmartly-preview-header-host">Header</div><p>Content</p>'
      const result = compressHtml(html)
    ...
    it("should remove data-absmartly-preview attributes", () => {
      const html =
        '<div data-absmartly-preview-mode="true" data-absmartly-preview-variant="A">Content</div>'
      const result = compressHtml(html)
    ...
    it("should remove data-absmartly-original attributes", () => {
      const html =
        '<div data-absmartly-original-text="old" data-absmartly-modified="true">Content</div>'
      const result = compressHtml(html)
    ...
    it("should remove Framer framework attributes", () => {
      const html =
        '<div data-framer-hydrate-v2="true" data-framer-appear-id="abc" data-framer-name="Component">Content</div>'
      const result = compressHtml(html)
    ...
    it("should remove @font-face rules from styles", () => {
      const html =
        '<html><head><style>@font-face { font-family: "Test"; } body { color: red; }</style></head><body>Content</body></html>'
      const result = compressHtml(html)
    ...
    it("should remove @import rules from styles", () => {
      const html =
        '<html><head><style>@import url("test.css"); body { color: red; }</style></head><body>Content</body></html>'
      const result = compressHtml(html)
    ...
    it("should remove event handler attributes", () => {
      const html =
        '<button onclick="alert()" onmouseover="hover()">Click</button>'
      const result = compressHtml(html)
  ...
  describe("getSystemPrompt", () => {
    ...
    it("should return default system prompt with chunk retrieval docs interpolated when no override exists", async () => {
      jest
        .spyOn(SystemPromptEditor, "getSystemPromptOverride")
      ...
      expect(SystemPromptEditor.getSystemPromptOverride).toHaveBeenCalledTimes(
        1
      )
    })
    ...
    it("should return override system prompt with chunk retrieval docs when one exists", async () => {
      const customPrompt =
        "Custom prompt with {{CHUNK_RETRIEVAL_DOCUMENTATION}} placeholder"
      jest
        .spyOn(SystemPromptEditor, "getSystemPromptOverride")
        .mockResolvedValue(customPrompt)

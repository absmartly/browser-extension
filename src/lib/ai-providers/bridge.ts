...
import { unsafeConversationId, unsafeSessionId } from "~src/types/branded"
...
import { buildUserMessage, getSystemPrompt } from "./utils"
import {
...
export class BridgeProvider implements AIProvider {
  ...
  async generate(
    ...
    try {
      ...
      debugLog(
        "[Bridge] Input session:",
        JSON.stringify(options.conversationSession)
      )
      debugLog(
        "[Bridge] Input HTML:",
        html ? `${html.length} chars` : "undefined"
      )
      debugLog("[Bridge] Input pageUrl:", options.pageUrl)
      debugLog(
        "[Bridge] Input domStructure:",
        options.domStructure
          ? `${options.domStructure.length} chars`
          : "not provided"
      )
      ...
      let systemPromptToSend = await getSystemPrompt(
        this.getChunkRetrievalPrompt()
      )
      const bridgeProvider = getBridgeProviderName(this.config.aiProvider)

      const providerChanged =
        session?.provider && session.provider !== bridgeProvider
      if (providerChanged) {
        debugLog(
          "[Bridge] Provider changed from",
          session!.provider,
          "to",
          bridgeProvider,
      ...
      if (!session || !session.conversationId || providerChanged) {
        debugLog(
          "[Bridge] No session or no conversationId, creating new conversation"
        )
        ...
        if (!html) {
          throw new Error(
            "HTML is required for creating new bridge conversation"
          )
        }
        ...
        const structureText =
          options.domStructure || "(DOM structure not available)"
        debugLog(
          "[Bridge] Using pre-generated DOM structure:",
          structureText.substring(0, 100) + "..."
        )
...
curl -s "` +
          `${bridgeUrl}/conversations/${sessionId}` +
          `/chunk?selector=.actual-class-from-structure"

# Multiple selectors (comma-separated, URL-encode # as %23)
curl -s "` +
          `${bridgeUrl}/conversations/${sessionId}` +
          `/chunk?selectors=header,%23main-content,section"
` +
          "```" +
          `
...
1. ✅ Always use ` +
          "`${bridgeUrl}`" +
          ` for curl - this is the localhost bridge server
...
`
        debugLog(
          "[Bridge] Including DOM structure in system prompt (not full HTML)"
        )
        ...
        debugLog(
          `[Bridge] 📊 System prompt length: ${systemPromptToSend.length} characters`
        )
        debugLog(
          "[Bridge] ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        )
        ...
        debugLog(
          "[Bridge] About to create conversation with sessionId:",
          sessionId
        )
        const model =
        ...
        debugLog(
          "[Bridge] ✅ Conversation created with JSON schema and HTML stored on bridge"
        )
      ...
      const result = await new Promise<AIDOMGenerationResult>(
        (resolve, reject) => {
          let fullResponse = ""
          ...
          debugLog(
            "[Bridge] Bridge connection URL:",
            this.bridgeClient.getConnection()?.url
          )
                ...
                if (event.type === "tool_use") {
                  ...
                  if (typeof toolInput === "string") {
                    try {
                      ...
                      debugWarn(
                        "[Bridge] Failed to parse tool_use data as JSON:",
                  ...
                  if (
                    toolInput &&
                    (toolInput.domChanges !== undefined ||
                      toolInput.response !== undefined ||
                      toolInput.action !== undefined)
                  ) {
                    debugLog(
                      "[Bridge] 📦 Received final dom_changes_generator result"
                    )
                    ...
                    debugLog(
                      "[Bridge] ℹ️ Intermediate tool call (handled by Claude Code internally)"
                    )
                  }
                  ...
                  if (finalToolResult) {
                    const validation = validateAIDOMGenerationResult(
                      JSON.stringify(finalToolResult)
                    )
                    if (validation.isValid) {
                      ...
                      reject(
                        new Error(
                          `Validation failed: ${(validation as ValidationError).errors.join(", ")}`
                        )
                      )
                    }
                    return
                  ...
                  if (typeof errorData === "string") {
                    try {
                      ...
                      debugWarn(
                        "[Bridge] Failed to parse SSE error data as JSON, using raw string:",
                  ...
                  const formatBridgeError = (value: unknown): string => {
                    ...
                    if (typeof value === "string") {
                      ...
                      if (obj.type || obj.code || obj.message) {
                        const prefix = [obj.type, obj.code]
                          .filter(Boolean)
                          .join(" ")
                        raw =
                          prefix && obj.message
                            ? `${prefix}: ${obj.message}`
                            : prefix || obj.message || String(value)
                      } else {
                        try {
                  ...
                  if (
                    ...
                    userMessage =
                      "Cannot connect to AI CLI Bridge. Make sure it's running:\n\nnpx @absmartly/ai-cli-bridge"
                  } else if (
                    ...
                    userMessage =
                      "Authentication failed. Check your Claude API credentials in Settings."
                  } else if (
                    ...
                    userMessage =
                      "Request timed out. Try again or simplify your request."
                  } else if (
          ...
          const sendMessage = async () => {
            try {
              await this.bridgeClient.sendMessage(
                conversationId,
                userMessage,
                images || [],
                systemPromptToSend,
      ...
      if (
        ...
        userMessage =
          "Request timed out after 5 minutes. Try again or simplify your request."
      } else if (

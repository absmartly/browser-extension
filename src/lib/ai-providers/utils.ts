...
export function compressHtml(html: string): string {
  ...
  if (plasmoLoadingStart !== -1) {
    ...
    if (plasmoEnd !== -1) {
      debugLog(
        "[Compress] Removing Plasmo loading div:",
        plasmoEnd - plasmoLoadingStart,
        "characters"
      )
      compressed =
        compressed.substring(0, plasmoLoadingStart) +
        compressed.substring(plasmoEnd)
    }
  }
  ...
  debugLog(
    "[Compress] After other Plasmo removal:",
    compressed.length,
    "Removed:",
    beforeOtherPlasmo - compressed.length
  )

  compressed = compressed
    .replace(
      /<iframe[^>]*id="absmartly-sidebar-iframe"[^>]*>[\s\S]*?<\/iframe>/gi,
      ""
    )
    .replace(/<iframe[^>]*chrome-extension[^>]*>[\s\S]*?<\/iframe>/gi, "")
    .replace(
      /<div[^>]*id="absmartly-preview-header-host"[^>]*>[\s\S]*?<\/div>/gi,
      ""
    )
  ...
  compressed = compressed.replace(
    ...
    (match, headContent) => {
      let styleTags = (
        headContent.match(/<style\b[^<]*(?:(?!<\/style>)<[^<]*)*<\/style>/gi) ||
        []
      )
        .filter((tag) => {
          return (
        ...
        .map((tag) => {
          ...
          cleaned = cleaned.replace(
            /\/\*\s*(vietnamese|latin-ext|latin|cyrillic-ext|cyrillic|greek)\s*\*\//gi,
            ""
          )
      ...
      debugLog(
        "[Compress] Found",
        styleTags.length,
        "essential style tags in head"
      )
      return styleTags.length > 0
        ? `<head>${styleTags.join("")}</head>`
        : "<head></head>"
    }
  )
  debugLog(
    "[Compress] After head strip:",
    compressed.length,
    "Removed:",
    beforeHeadStrip - compressed.length
  )

  return compressed
    .replace(
      /<div[^>]*id="__framer-editorbar-container"[^>]*>[\s\S]*?<\/div>/gi,
      ""
    )
    .replace(
      /<iframe[^>]*id="__framer-editorbar"[^>]*>[\s\S]*?<\/iframe>/gi,
      ""
    )
    .replace(/<div[^>]*id="tooltip-root"[^>]*>[\s\S]*?<\/div>/gi, "")
    .replace(
      /<div[^>]*id="hs-web-interactives-[^"]*"[^>]*>[\s\S]*?<\/div>/gi,
      ""
    )
    .replace(/<div[^>]*id="tldx-[^"]*"[^>]*>[\s\S]*?<\/div>/gi, "")
    .replace(
      /<tldx-lmi-shadow-root[^>]*>[\s\S]*?<\/tldx-lmi-shadow-root>/gi,
      ""
    )
  ...
  try {
    ...
    const preview =
      rawArguments.length > 200
        ? rawArguments.slice(0, 200) + "..."
        : rawArguments
    throw new Error(
      `[${providerName}] Failed to parse arguments for tool "${toolName}": ${preview}`
    )
  }
}
...
export async function getSystemPrompt(
  ...
  const result = basePrompt.replace(
    "{{CHUNK_RETRIEVAL_DOCUMENTATION}}",
    chunkRetrievalPrompt
  )
...
export function buildUserMessage(
  ...
  if (currentChanges.length > 0) {
    const sanitizedChanges = sanitizeHtml(
...
export function buildSystemPromptWithDOMStructure(
  ...
  debugLog(
    `[${providerName}] Using pre-generated DOM structure:`,
...
`
  ...
  debugLog(
    `[${providerName}] Including DOM structure in system prompt (${domStructure.length} chars)`
  )

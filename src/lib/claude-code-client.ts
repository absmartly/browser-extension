...
Common issues:
  ...
  async connect(): Promise<BridgeConnection> {
    ...
    try {
      const customEndpoint = await this.storage.get<string>(
        STORAGE_KEY_BRIDGE_ENDPOINT
      )
      ...
      if (customEndpoint) {
        ...
        debugLog(
          `[Bridge] ✅ Connected to custom endpoint ${url}`,
          this.connection
        )
  ...
  async refreshHtml(conversationId: string, html: string): Promise<void> {
    ...
    const response = await fetch(
      `${this.connection!.url}/conversations/${conversationId}/refresh`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async getHtmlChunks(
    ...
    const response = await fetch(
      `${this.connection!.url}/conversations/${conversationId}/chunks`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async queryXPath(
    ...
    const response = await fetch(
      `${this.connection!.url}/conversations/${conversationId}/xpath`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async sendMessage(
    ...
    const response = await fetch(
      `${this.connection!.url}/conversations/${conversationId}/messages`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  streamResponses(
    ...
    eventSource.onmessage = (event) => {
      ...
      try {
        ...
        debugWarn(
          "[Bridge] Skipping malformed SSE message:",
      ...
      switch (readyState) {
        case 0: // CONNECTING
          ...
          if (reconnectAttempts < MAX_RECONNECT_ATTEMPTS) {
            ...
            debugLog(
              "[Bridge] Allowing reconnection attempt:",
              reconnectAttempts
            )
            return
          } else {
        ...
        case 2: // CLOSED
          errorMessage =
            "Stream connection closed. The bridge server may have stopped or the conversation may have ended."
          break
        default:

...
export function variantName(name: string): VariantName {
  if (typeof name !== "string" || name.trim().length === 0) {
    throw new Error(
      `Invalid variant name: ${name}. Must be a non-empty string.`
    )
  }
...
export function cssSelector(selector: string): CSSSelector {
  if (typeof selector !== "string" || selector.trim().length === 0) {
    throw new Error(
      `Invalid CSS selector: ${selector}. Must be a non-empty string.`
    )
  }
  try {
...
export function xpathSelector(xpath: string): XPathSelector {
  if (typeof xpath !== "string" || xpath.trim().length === 0) {
    throw new Error(
      `Invalid XPath selector: ${xpath}. Must be a non-empty string.`
    )
  }
...
export function apiEndpoint(url: string): APIEndpoint {
  ...
  try {
    ...
    throw new Error(
      `Invalid API endpoint URL: ${url}. ${error instanceof Error ? error.message : ""}`
    )
  }
...
export function applicationId(id: number): ApplicationId {
  if (!Number.isInteger(id) || id <= 0) {
    throw new Error(
      `Invalid application ID: ${id}. Must be a positive integer.`
    )
  }
...
export function conversationId(id: string): ConversationId {
  if (typeof id !== "string" || id.trim().length === 0) {
    throw new Error(
      `Invalid conversation ID: ${id}. Must be a non-empty string.`
    )
  }

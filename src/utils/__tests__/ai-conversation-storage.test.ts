...
describe("ai-conversation-storage", () => {
  ...
  describe("getConversations", () => {
    it("should delegate to idbStorage.getConversations", async () => {
      ...
      expect(mockIdbStorage.getConversations).toHaveBeenCalledWith(
  ...
  describe("saveConversation", () => {
    ...
    it("should throw quota exceeded error when storage fails with quota message", async () => {
      mockIdbStorage.saveConversation.mockRejectedValue(
  ...
  describe("loadConversation", () => {
    it("should delegate to idbStorage.loadConversation", async () => {
      ...
      expect(mockIdbStorage.loadConversation).toHaveBeenCalledWith(
        "variant-a",
        "conv-123"
      )
  ...
  describe("deleteConversation", () => {
    it("should delegate to idbStorage.deleteConversation", async () => {
      ...
      expect(mockIdbStorage.deleteConversation).toHaveBeenCalledWith(
        "variant-a",
        "conv-123"
      )
    })
    ...
    it("should re-throw errors", async () => {
      ...
      await expect(deleteConversation("variant-a", "conv-123")).rejects.toThrow(
        "DB error"
      )
    })
  })
  ...
  describe("getConversationList", () => {
    it("should delegate to idbStorage.getConversationList", async () => {
      ...
      expect(mockIdbStorage.getConversationList).toHaveBeenCalledWith(
        unsafeVariantName("variant-a")
      )
    ...
    it("should return empty array on error", async () => {
      mockIdbStorage.getConversationList.mockRejectedValue(
        new Error("DB error")
      )
  ...
  describe("setActiveConversation", () => {
    it("should delegate to idbStorage.setActiveConversation", async () => {
      ...
      expect(mockIdbStorage.setActiveConversation).toHaveBeenCalledWith(
        "variant-a",
        "conv-123"
      )
    })
    ...
    it("should re-throw errors", async () => {
      mockIdbStorage.setActiveConversation.mockRejectedValue(
        new Error("DB error")
      )

      await expect(
        setActiveConversation("variant-a", "conv-123")

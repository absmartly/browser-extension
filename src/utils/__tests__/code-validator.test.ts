...
import {
  validateExperimentCode,
  validateExperimentURL
} from "../code-validator"
  ...
  describe("validateExperimentCode", () => {
    describe("valid safe code", () => {
      ...
      it("should allow array operations", () => {
        const result = validateExperimentCode(
          "var arr = [1, 2, 3]; arr.forEach(x => console.log(x));"
        )
        expect(result.valid).toBe(true)
      })
    ...
    describe("edge cases and complex scenarios", () => {
      ...
      it("should block multiple violations", () => {
        const result = validateExperimentCode(
          'eval("x"); fetch("url"); document.cookie;'
        )
        expect(result.valid).toBe(false)
      })
      ...
      it("should block constructor even in safe-looking code", () => {
        const result = validateExperimentCode(
          "var safe = obj.constructor.prototype;"
        )
        expect(result.valid).toBe(true)
        expect(result.warnings).toBeDefined()
      })
  ...
  describe("validateExperimentURL", () => {
    describe("valid URLs", () => {
      ...
      it("should allow URL with query parameters", () => {
        const result = validateExperimentURL(
          "https://example.com/api?key=value&id=123"
        )
        expect(result.valid).toBe(true)
      })
    ...
    describe("SSRF prevention", () => {
      ...
      it("should block 172.16.x.x through 172.31.x.x", () => {
        ...
        expect(validateExperimentURL("http://172.31.255.255/").valid).toBe(
          false
        )
      })
    ...
    describe("edge cases", () => {
      ...
      it("should allow CDN URLs", () => {
        expect(
          validateExperimentURL("https://cdn.jsdelivr.net/npm/package").valid
        ).toBe(true)
        expect(validateExperimentURL("https://unpkg.com/package").valid).toBe(
          true
        )
        expect(
          validateExperimentURL("https://cdnjs.cloudflare.com/ajax/libs/").valid
        ).toBe(true)
      })
    })
  })

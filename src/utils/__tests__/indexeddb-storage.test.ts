...
import { closeDatabase, openDatabase } from "~src/utils/indexeddb-connection"
...
describe("IndexedDB Storage Operations", () => {
  ...
  describe("CRUD Operations", () => {
    it("should save and retrieve a conversation", async () => {
      ...
      const loaded = await idbStorage.loadConversation(
        unsafeVariantName("test-variant"),
        unsafeConversationId("conv-1")
      )
    ...
    it("should delete a conversation", async () => {
      ...
      await idbStorage.saveConversation(conversation)
      await idbStorage.deleteConversation(
        unsafeVariantName("test-variant"),
        unsafeConversationId("conv-1")
      )
    ...
    it("should get all conversations for a variant", async () => {
      await idbStorage.saveConversation(
        createTestConversation("variant-a", "conv-1")
      )
      await idbStorage.saveConversation(
        createTestConversation("variant-a", "conv-2")
      )
      await idbStorage.saveConversation(
        createTestConversation("variant-b", "conv-3")
      )

      const conversations = await idbStorage.getConversations(
        unsafeVariantName("variant-a")
      )

      expect(conversations.length).toBe(2)
      expect(conversations.every((c) => c.variantName === "variant-a")).toBe(
        true
      )
    })
  })
  ...
  describe("Conversation List", () => {
    it("should return conversation list with metadata only", async () => {
      ...
      const list = await idbStorage.getConversationList(
        unsafeVariantName("variant-a")
      )
  ...
  describe("Active Conversation", () => {
    it("should set active conversation", async () => {
      ...
      await idbStorage.setActiveConversation(
        unsafeVariantName("variant-a"),
        unsafeConversationId("conv-2")
      )

      const conv1 = await idbStorage.loadConversation(
        unsafeVariantName("variant-a"),
        unsafeConversationId("conv-1")
      )
      const conv2 = await idbStorage.loadConversation(
        unsafeVariantName("variant-a"),
        unsafeConversationId("conv-2")
      )
    ...
    it("should deactivate previous active conversation", async () => {
      ...
      const updated1 = await idbStorage.loadConversation(
        unsafeVariantName("variant-a"),
        unsafeConversationId("conv-1")
      )
      const updated2 = await idbStorage.loadConversation(
        unsafeVariantName("variant-a"),
        unsafeConversationId("conv-2")
      )
  ...
  describe("Error Handling", () => {
    it("should handle loading non-existent conversation gracefully", async () => {
      const conversation = await idbStorage.loadConversation(
        unsafeVariantName("variant-a"),
        unsafeConversationId("non-existent")
      )
    ...
    it("should handle getting conversations from non-existent variant", async () => {
      const conversations = await idbStorage.getConversations(
        unsafeVariantName("non-existent-variant")
      )
    ...
    it("should handle getting list from empty variant", async () => {
      const list = await idbStorage.getConversationList(
        unsafeVariantName("empty-variant")
      )
      expect(list).toEqual([])
    })
  })

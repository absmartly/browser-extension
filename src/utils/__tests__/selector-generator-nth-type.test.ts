...
describe('Real-world absmartly.com scenarios', () => {
  ...
  test('Case 1: Features section - "Discuss experiments" paragraph', () => {
    ...
    expect(target).toBeTruthy()
    expect(target.textContent).toContain('Discuss experiments in context with a built-in social feed')
...
describe('Target with semantic attributes but no semantic ancestors', () => {
  ...
  test('should use semantic attribute with positional ancestors when no semantic ancestors exist', () => {
    const target = document.querySelector('[data-test-target="3"]') as HTMLElement
    expect(target).toBeTruthy()
    ...
    console.log('Generated selector for Benefits (no semantic ancestors):', selector)
    ...
    expect(selector).not.toBe('div:nth-of-type(2) > div:nth-of-type(1) > div:nth-of-type(3)')

...
import { sanitizeSelector, validateSelector } from "../selector-validator"

beforeAll(() => {
  const originalQuerySelector = document.querySelector
  document.querySelector = jest.fn((selector: string) => {
    const hasUnmatchedBracket =
      (selector.match(/\[/g) || []).length !==
      (selector.match(/\]/g) || []).length
    const hasUnmatchedParen =
      (selector.match(/\(/g) || []).length !==
      (selector.match(/\)/g) || []).length
...
describe("CSS Selector Validator", () => {
  describe("validateSelector", () => {
    describe("valid CSS selectors", () => {
      ...
      it("should allow complex selector", () => {
        expect(validateSelector("div.container > ul.list li:first-child")).toBe(
          true
        )
      })
    ...
    describe("XSS and injection prevention", () => {
      ...
      it("should block data:text/html with uppercase", () => {
        expect(validateSelector('iframe[src="DATA:TEXT/HTML,payload"]')).toBe(
          false
        )
      })
    ...
    describe("edge cases", () => {
      ...
      it("should allow complex real-world selector", () => {
        expect(
          validateSelector('div.container > ul#menu li.active a[class="link"]')
        ).toBe(true)
      })
  ...
  describe("integration scenarios", () => {
    ...
    it("should block common XSS attack patterns", () => {
      ...
      expect(validateSelector('iframe[src="data:text/html,<script>"]')).toBe(
        false
      )
    })

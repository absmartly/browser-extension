...
import { sanitizeXPath, validateXPath } from "../xpath-validator"
...
describe("XPath Validator", () => {
  describe("validateXPath", () => {
    describe("valid XPath expressions", () => {
      ...
      it("should allow XPath with multiple attributes", () => {
        expect(
          validateXPath("//div[@class='container'][@data-test='value']").valid
        ).toBe(true)
      })

      it("should allow XPath with contains function", () => {
        expect(validateXPath("//div[contains(@class, 'test')]").valid).toBe(
          true
        )
      })
      ...
      it("should allow XPath with starts-with function", () => {
        expect(validateXPath("//div[starts-with(@class, 'btn')]").valid).toBe(
          true
        )
      })
      ...
      it("should allow complex XPath with contains function", () => {
        expect(
          validateXPath(
            "//div[@class='container']//span[contains(@class, 'text')]"
          ).valid
        ).toBe(true)
      })
    })
    ...
    describe("dangerous pattern detection", () => {
      ...
      it("should block XPath targeting sensitive data", () => {
        expect(validateXPath("//div[@class='sensitive-data']").valid).toBe(
          false
        )
      })
      ...
      it("should block password in middle of XPath", () => {
        expect(
          validateXPath("//div//input[@type='password']//span").valid
        ).toBe(false)
      })
    })
    ...
    describe("edge cases", () => {
      ...
      it("should handle XPath with spaces in attribute values", () => {
        expect(validateXPath("//div[@class='btn btn-primary']").valid).toBe(
          true
        )
      })
      ...
      it("should allow XPath with parentheses and commas in contains/starts-with", () => {
        expect(validateXPath("//div[contains(text(), 'test')]").valid).toBe(
          true
        )
      })
  ...
  describe("integration scenarios", () => {
    ...
    it("should block common attack patterns", () => {
      ...
      expect(validateXPath("//div<img src=x onerror=alert(1)>").valid).toBe(
        false
      )
    })
  })
  ...
  describe("XPath injection prevention", () => {
    describe("SQL-style injection attempts", () => {
      ...
      it("should block OR true() injection", () => {
        expect(validateXPath("//user[name='admin' or true()]").valid).toBe(
          false
        )
      })

      it("should block AND 1=1 injection with double quotes", () => {
        expect(validateXPath('//user[name="admin" and "1"="1"]').valid).toBe(
          false
        )
      })
      ...
      it("should block injection with whitespace variations", () => {
        ...
        expect(validateXPath("//user[name='admin' OR '1'='1']").valid).toBe(
          false
        )
      })
    })
    ...
    describe("dangerous XPath functions", () => {
      ...
      it("should block normalize-space() function", () => {
        expect(validateXPath("//div[normalize-space(text())]").valid).toBe(
          false
        )
      })
      ...
      it("should block translate() function", () => {
        expect(validateXPath("//div[translate(@id, 'abc', 'xyz')]").valid).toBe(
          false
        )
      })
    ...
    describe("complex injection scenarios", () => {
      it("should block nested injection attempts", () => {
        expect(
          validateXPath(
            "//user[name='admin']//password[text()='secret' or '1'='1']"
          ).valid
        ).toBe(false)
      })
      ...
      it("should block multiple boolean logic chains", () => {
        expect(
          validateXPath("//user[id='1' and '1'='1' or '2'='2']").valid
        ).toBe(false)
      })
    })
    ...
    describe("edge cases that should still work", () => {
      ...
      it("should allow nested element selectors", () => {
        expect(validateXPath("//div//span//a").valid).toBe(true)
        expect(
          validateXPath("//div[@id='main']//button[@class='submit']").valid
        ).toBe(true)
      })
    })
  })

...
export async function needsMigration(
  ...
  try {
    ...
    console.error(
      "[ConversationMigration] Error checking migration need:",
      error
    )
    return false
  }
...
export async function migrateConversation(
  ...
  try {
    ...
    if (!Array.isArray(oldMessages) || oldMessages.length === 0) {
      debugLog(
        "[ConversationMigration] Old data is empty or invalid, skipping migration"
      )
    ...
    const firstUserMessage =
      oldMessages.find((m) => m.role === "user")?.content ||
    ...
    debugLog(
      `[ConversationMigration] Successfully migrated conversation for ${variantName}`
    )

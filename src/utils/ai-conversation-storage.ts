...
async function sanitizeConversationForStorage(
...
export async function saveConversation(
  ...
  try {
    ...
    debugLog(
      `[ConversationStorage] Saved conversation ${conversation.id} for ${conversation.variantName}`
    )
  } catch (error) {
    ...
    if (error instanceof Error && error.message.includes("quota")) {
      throw new Error(
        "Storage quota exceeded. Please start a new conversation or delete old conversations."
      )
    }
...
export async function getConversationList(
  ...
  try {
    ...
    console.error(
      "[ConversationStorage] Error getting conversation list:",
      error
    )
    return []
  }
...
export async function setActiveConversation(
  ...
  try {
    ...
    debugLog(
      `[ConversationStorage] Set active conversation to ${conversationId}`
    )
  } catch (error) {
    console.error(
      "[ConversationStorage] Error setting active conversation:",
      error
    )
    throw error
  }

 ...
 */
export function parseCookieFormat(cookieValue: string): {
 ...
 * New format uses comma to separate experiments and dot to separate values
 */
export function serializeToCookieFormat(
  overrides: ExperimentOverrides,
  devEnv?: string | null
): string {
...
export function setCookieOverridesScript(
  overrides: ExperimentOverrides
): string {
...
export async function saveDevelopmentEnvironment(
  ...
  try {
    ...
    debugWarn(
      "[Overrides] Failed to sync dev environment to cookie (non-critical):",
      error
    )
  }
}
 ...
 */
async function syncOverridesToCookie(
  overrides: ExperimentOverrides
): Promise<void> {
  try {
    ...
    debugLog(
      "[ABsmartly] Syncing to cookie - overrides:",
      overrides,
      "devEnv:",
      devEnv
    )
    ...
    if (typeof chrome !== "undefined" && chrome.tabs && chrome.tabs.query) {
      ...
      const cookieValue =
        Object.keys(overrides).length === 0 && !devEnv
          ? ""
      ...
      if (Object.keys(overrides).length === 0) {
        document.cookie =
          "absmartly_overrides=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;"
      } else {

...
declare global {
  interface Window {
    __absmartlyGetVariantAssignments?: (
 ...
 */
export async function getCurrentVariantAssignments(
  experimentNames: string[]
): Promise<SDKVariantData> {
  try {
    ...
    const result = await chrome.scripting.executeScript({
      ...
      func: async (expNames) => {
        ...
        if (typeof window.__absmartlyGetVariantAssignments === "function") {
          return await window.__absmartlyGetVariantAssignments(
            expNames as string[]
          )
        }
 ...
 */
export async function getSDKContextPath(): Promise<{
  found: boolean
  path: string | null
}> {
  try {
    ...
    const result = await chrome.scripting.executeScript({
      ...
      func: () => {
        ...
        if (
          ...
          typeof window.ABsmartlyContext.peek === "function"
        ) {
    ...
    return (
      (result[0]?.result as { found: boolean; path: string | null }) || {
        found: false,
        path: null
      }
    )

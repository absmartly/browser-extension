    ...
    if (!looksNormal) {
      ...
      if (segment.length <= 5) {
        ...
        if (/\d/.test(segment) && /[a-zA-Z]/.test(segment)) {
          ...
          if (
            !/^[a-z]+\d{1,2}$/.test(segment) && // Not like "test1"
            !/^\d{1,2}[a-z]+$/.test(segment)
          ) {
            // Not like "1st"
            return true
          }
        ...
        if (
          /[0-9][a-zA-Z]{2,}[0-9]/.test(segment) || // number-letters-number
...
export function getSelector(element: Element): string {
      ...
      if (parent.id && !isAutoGenerated(parent.id)) {
        ...
        try {
          ...
          if (matches.length === 1 && matches[0] === element) {
            console.log(
              "[ABSmartly] Using duplicate ID with parent context (fast):",
              selector
            )
            return selector
          }
    ...
    if (parent.id && !isAutoGenerated(parent.id)) {
      ...
      for (let j = i - 1; j >= 0; j--) {
        ...
        if (currentElem.classList && currentElem.classList.length > 0) {
          const uniqueClass = Array.from(currentElem.classList).find((cls) => {
            if (isAutoGenerated(cls)) return false
            const classMatches = document.querySelectorAll(
              "." + escapeSelector(cls)
            )
      ...
      try {
        ...
        if (matches.length === 1 && matches[0] === element) {
          console.log(
            "[ABSmartly] Using parent ID with tag path (moderately fast):",
            selector
          )
          return selector
        }
        ...
        if (element.id && !isAutoGenerated(element.id) && matches.length > 1) {
          ...
          if (idMatches.length === 1 && idMatches[0] === element) {
            console.log(
              "[ABSmartly] Using parent ID with duplicate child ID:",
              selector
            )
            return selector
          }
        ...
        for (let i = 1; i < pathElements.length; i++) {
          ...
          if (parent.id && !isAutoGenerated(parent.id)) {
            ...
            if (parentMatches.length === 1 && parentMatches[0] === element) {
              console.log(
                "[ABSmartly] Using class with parent ID:",
                parentClassSelector
              )
            ...
            if (tagMatches.length === 1 && tagMatches[0] === element) {
              console.log(
                "[ABSmartly] Using tag+class with parent ID:",
                tagClassSelector
              )
  ...
  const preferredAttr = dataAttrs.find((attr) =>
    preferredDataAttrs.includes(attr.name)
  )

  if (preferredAttr) {
    const dataSelector =
      "[" + preferredAttr.name + '="' + preferredAttr.value + '"]'
    try {
      const matches = document.querySelectorAll(dataSelector)
      if (matches.length === 1 && matches[0] === element) {
        console.log(
          "[ABSmartly] Using data attribute (slower but stable):",
          dataSelector
        )
        return dataSelector
      }
      ...
      for (let i = 1; i < pathElements.length; i++) {
        ...
        if (parent.id && !isAutoGenerated(parent.id)) {
          ...
          if (parentMatches.length === 1 && parentMatches[0] === element) {
            console.log(
              "[ABSmartly] Using data attribute with parent ID:",
              parentDataSelector
            )
  ...
  for (let startDepth = 1; startDepth <= pathElements.length; startDepth++) {
      ...
      if (elem.id && !isAutoGenerated(elem.id)) {
        ...
        for (let j = i - 1; j >= 0; j--) {
          ...
          if (childElem.classList && childElem.classList.length > 0) {
            const uniqueClass = Array.from(childElem.classList).find((cls) => {
              if (isAutoGenerated(cls)) return false
              // Only use if it helps distinguish among siblings
              if (childElem.parentElement) {
                const siblings = Array.from(
                  childElem.parentElement.children
                ).filter(
                  (s) => s.tagName === childElem.tagName && s !== childElem
                )
        ...
        if (elem.classList && elem.classList.length > 0) {
          const uniqueClass = Array.from(elem.classList).find((cls) => {
            if (isAutoGenerated(cls)) return false
            const classMatches = document.querySelectorAll(
              "." + escapeSelector(cls)
            )
    ...
    if (matches.length === 1 && matches[0] === element) {
      ...
      if (
        ...
        console.log(
          "[ABSmartly] Single tag selector too brittle, continuing search:",
          selector
        )
        continue
      }
  ...
  if (tagMatches.length === 1 && tagMatches[0] === element) {
    ...
    if (
      ...
      if (
        ...
        if (parentMatches.length === 1 && parentMatches[0] === element) {
          console.log(
            "[ABSmartly] Added parent context to structural tag:",
            parentSelector
          )
    ...
    if (parent.id && !isAutoGenerated(parent.id)) {
      ...
      for (let i = parentIdx - 1; i >= 0; i--) {
        ...
        if (elem.parentElement) {
          const sameTags = Array.from(elem.parentElement.children).filter(
            (child) => child.tagName === elem.tagName
          )
      ...
      try {
        ...
        if (matches.length === 1 && matches[0] === element) {
          console.log(
            "[ABSmartly] Using nth-of-type selector (slow but unique):",
            fullSelector
          )
          return fullSelector
        }

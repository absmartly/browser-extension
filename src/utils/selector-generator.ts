...
function buildContextualSelector(element: Element, maxParentLevels: number): string | null {
  ...
  for (const {element: ancestor} of semanticAncestors) {
    ...
    if (matches.length > 1 && matches.length < 20) {
      ...
      while (checkDepth <= maxCheckDepth) {
        ...
        if (semanticAttr && !isAutoGenerated(semanticAttr)) {
          // Check if other matching elements have different ancestors at this level
          const allSame = ancestorsAtDepth.every(a => a === ourAncestor)
          if (!allSame) {
            disambiguatingAncestor = ourAncestor
            break
          }
        }
      ...
      // If we found a disambiguating ancestor, use it
      if (disambiguatingAncestor) {
        const disambiguatingPart = getSelectorPart(disambiguatingAncestor, false)
        if (disambiguatingPart) {
          const disambiguatedSelector = `${disambiguatingPart} ${selector}`
          const finalMatches = document.querySelectorAll(disambiguatedSelector)

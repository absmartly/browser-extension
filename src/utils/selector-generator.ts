...
function isAutoGenerated(str: string): boolean {
  ...
  for (const segment of segments) {
    ...
    // 1. Mixed case with no clear camelCase pattern
    // 2. Numbers randomly interspersed
    // 3. Short length (6-12 chars) but high entropy
    ...
    // High entropy indicator: has all three types (upper, lower, digits) in short string
    const hasAllTypes = upperCount > 0 && lowerCount > 0 && digitCount > 0
    const isShortMixed = segment.length <= 12 && hasAllTypes
    ...
    // Random: multiple irregular transitions
    const caseTransitions = (segment.match(/[a-z][A-Z]/g) || []).length
    const hasIrregularCasing = caseTransitions > 1 || (caseTransitions === 1 && /^[a-z].*[A-Z].*[0-9]/.test(segment))

    // Detect random alphanumeric patterns like "qm7gSEyfY"
    const randomAlphanumericPattern = /^[a-zA-Z0-9]{7,12}$/ // 7-12 chars, alphanumeric only
    const matchesRandomPattern = randomAlphanumericPattern.test(segment)

    if (matchesRandomPattern && (isShortMixed || hasIrregularCasing)) {
      return true
    }

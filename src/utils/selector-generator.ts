...
function buildContextualSelector(element: Element, maxParentLevels: number): string | null {
  ...
  if (semanticAncestors.length === 0 && targetPart) {
    ...
    let parent = element.parentElement
    let disambiguatedSelector = targetPart
    let depth = 1
    const maxAttempts = 5
    ...
    while (parent && depth <= maxAttempts) {
      ...
      if (siblings.length > 1) {
        ...
        parentSelector = `${parentTag}:nth-of-type(${index})`
      } else {
        parentSelector = parentTag
      }

      // Try with child combinator for precision
        ...
        if (semanticAttr && !isAutoGenerated(semanticAttr)) {
          ...
          if (otherHaveDifferentValues) {
            ...
            if (disambiguatingPart) {
              // Check DOM hierarchy: is disambiguating ancestor ABOVE or BELOW the semantic parent?
              // Walk up from semantic parent (ancestor from outer loop) to see if we hit disambiguating ancestor
              const isDisambiguatingAboveSemanticParent = (() => {
                let current = ancestor as Element | null
                while (current && current !== document.body) {
                  if (current === ourAncestor) return true
                  current = current.parentElement
                }
                return false
              })()
              ...
              let withSemanticParent: string
              if (isDisambiguatingAboveSemanticParent) {
                // Disambiguating is higher in tree: disambiguatingAncestor semanticParent target
                withSemanticParent = `${disambiguatingPart} ${parentPart} ${targetPart || targetTag}`
              } else {
                // Semantic parent is higher in tree: semanticParent disambiguatingAncestor target
              ...
              // Strategy 1.5: If simple combination didn't work, try building a precise chain
              // from semantic parent through disambiguating ancestor to target
              if (semanticMatches.length > 1) {
                // Build a precise chain from ancestor (the outer loop's semantic parent) to target
                let preciseChain = parentPart

                // Build path from semantic parent to target
                const chainPath: Element[] = []
                let tempEl: Element | null = element
                while (tempEl && tempEl !== ancestor) {
                  chainPath.unshift(tempEl)
                ...
                if (chainPath.length > 0) {
                  ...
                  for (let i = 0; i < chainPath.length; i++) {
                    const pathElement = chainPath[i]
                    const parent = i === 0 ? ancestor : chainPath[i - 1]
                    ...
                    // Check if semanticPart has real semantic info (not just a tag name)
              ...
              // Strategy 2: Try without the intermediate semantic parent (direct descendant)
              ...
              // Don't immediately fall back to full chain - continue looking for better disambiguating ancestors
              // The full chain will be built as a last resort after trying all semantic parents
            }
          }
        }
  ...
  for (const { element: semanticAncestor } of semanticAncestors) {
    ...
    // Strategy 3: Build full chain from semantic ancestor to target
    let currentSelector = parentPart

    // Build path from semantic ancestor to target
    const pathToTarget: Element[] = []
    let tempElement: Element | null = element
    while (tempElement && tempElement !== semanticAncestor) {
    ...
    for (let i = 0; i < pathToTarget.length; i++) {
      ...
      const parent = i === 0 ? semanticAncestor : pathToTarget[i - 1]

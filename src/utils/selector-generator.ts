...
function isAutoGenerated(str: string): boolean {
  ...
  // Check for framework-specific patterns first
  if (AUTO_GENERATED_PATTERNS.some(pattern => pattern.test(str))) {
    return true
  }

  // Generic check: Look for sequences of random-looking characters
  // Random sequences typically have:
  // - Mixed case without following camelCase/PascalCase patterns
  // - Numbers interspersed with letters randomly
  ...
  for (const segment of segments) {
    ...
    // Check if this segment looks like a normal word or naming pattern
    const looksLikeWord = /^[A-Z][a-z]+$/.test(segment) || // PascalCase word
                          /^[a-z]+$/.test(segment) ||        // all lowercase
                          /^[A-Z]+$/.test(segment) ||        // all uppercase (could be acronym)
                          /^[a-z]+[0-9]{1,2}$/.test(segment) || // word with 1-2 digit suffix
                          /^[A-Z][a-z]+[0-9]{1,2}$/.test(segment) // PascalCase with 1-2 digit suffix

    if (looksLikeWord) continue

    // Improved detection for random-looking alphanumeric strings like "qm7gSEyfY"
    // These have characteristics:
    // 1. Mixed case with no clear camelCase/PascalCase pattern
    // 2. Numbers interspersed in the middle (not just suffix)
    // 3. Short length (7-12 chars) but high entropy
    ...
    // Check if digits are interspersed (not just at the end)
    // Valid: "userId123", "button2", "Item1"
    // Invalid: "qm7gSEyfY", "a2B3c4D"
    const hasDigitsInMiddle = /[0-9].*[a-zA-Z]/.test(segment)

    // Check for irregular case mixing (not camelCase/PascalCase)
    // camelCase: starts lowercase, has one uppercase transition
    // PascalCase: starts uppercase, may have more transitions
    // Random: multiple irregular transitions OR digits interspersed with case changes
    const caseTransitions = (segment.match(/[a-z][A-Z]/g) || []).length

    // Multiple case transitions = likely random (e.g., "qmSEyfY" has 2 transitions)
    const hasMultipleTransitions = caseTransitions > 1

    // Single transition but with interspersed digits = likely random (e.g., "qm7gSE")
    const hasSuspiciousPattern = caseTransitions === 1 && hasDigitsInMiddle

    // High entropy: all three character types with digits in middle
    const hasAllTypes = upperCount > 0 && lowerCount > 0 && digitCount > 0
    const hasHighEntropy = segment.length >= 7 && segment.length <= 12 && hasAllTypes && hasDigitsInMiddle

    if (hasHighEntropy || hasMultipleTransitions || hasSuspiciousPattern) {
      return true
    }

    // Original detection patterns for longer sequences
    const randomSequencePattern = /[a-z][A-Z0-9]{2,}[a-z][A-Z0-9]|[0-9][a-zA-Z]{2,}[0-9]|[a-z]{2,}[A-Z]{2,}[a-z]|[A-Z][a-z][0-9][a-zA-Z]{4,}/
    if (randomSequencePattern.test(segment)) {
      return true
    }

    // Check for long mixed alphanumeric that doesn't follow patterns
    const mixedRandomPattern = /[a-z0-9]{8,}|[A-Z0-9]{8,}/
    if (segment.length >= 8 && mixedRandomPattern.test(segment)) {
      const hasRandomCaseMixing = /[a-z].*[A-Z].*[a-z]/.test(segment) ||
                                  /[0-9].*[a-zA-Z].*[0-9].*[a-zA-Z]/.test(segment)
      if (hasRandomCaseMixing) {
        return true
      }
    }
  }
...
function getDataAttributeSelector(element: Element): string | null {
  ...
  const preferredAttrs = ['data-testid', 'data-test', 'data-cy', 'data-id', 'data-name', 'data-framer-name']

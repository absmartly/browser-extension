...
function getDataAttributeSelector(element: Element): string | null {
  ...
  if (preferred && !isAutoGenerated(preferred.value)) {
    return `[${preferred.name}="${preferred.value}"]`
  }

  // Use the first non-auto-generated data attribute
  const goodAttr = dataAttrs.find(attr => !isAutoGenerated(attr.value))
  if (goodAttr) {
    return `[${goodAttr.name}="${goodAttr.value}"]`
  }
...
function getSelectorPart(element: Element, isTarget: boolean): string {
  ...
  if (isTarget) {
    ...
    // Just return the tag name if it's unique
    return tagName
  }

...
describe('ElementActions', () => {
  ...
  beforeEach(() => {
    ...
    // Mock ChangeTracker
    const MockedChangeTracker = ChangeTracker as jest.MockedClass<typeof ChangeTracker>
    mockChangeTracker = new MockedChangeTracker(mockStateManager) as jest.Mocked<ChangeTracker>
  ...
  describe('Element Manipulation - Delete', () => {
    ...
    it('should delete selected element', () => {
      ...
      expect(parent?.contains(testDiv)).toBe(false)
      expect(mockStateManager.setChanges).toHaveBeenCalled()
      expect(mockOptions.onChangesUpdate).toHaveBeenCalledWith(
        expect.arrayContaining([
          expect.objectContaining({
  ...
  describe('Private Methods - Change Management', () => {
    beforeEach(() => {
      // Reset changes to empty state
      const stateChangeCallback = mockStateManager.onStateChange.mock.calls[0][0]
      stateChangeCallback({
        ...mockStateManager.getState(),
        changes: []
    ...
    it('should add new change when none exists', () => {
      mockGenerateRobustSelector.mockReturnValue('#test-div')
    ...
    it('should add multiple different changes', () => {
      mockGenerateRobustSelector.mockReturnValue('#test-div')

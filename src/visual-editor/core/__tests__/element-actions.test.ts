...
describe("ElementActions", () => {
  ...
  let mockGenerateRobustSelector: jest.MockedFunction<
    typeof generateRobustSelector
  >
  ...
  beforeEach(() => {
    ...
    // Mock StateManager
    const MockedStateManager = StateManager as jest.MockedClass<
      typeof StateManager
    >
    ...
    const MockedUndoRedoManager = UndoRedoManager as jest.MockedClass<
      typeof UndoRedoManager
    >
    mockUndoRedoManager =
      new MockedUndoRedoManager() as jest.Mocked<UndoRedoManager>

    // Mock Notifications
    const MockedNotifications = Notifications as jest.MockedClass<
      typeof Notifications
    >
    mockNotifications = new MockedNotifications() as jest.Mocked<Notifications>

    // Mock selector generator
    mockGenerateRobustSelector = generateRobustSelector as jest.MockedFunction<
      typeof generateRobustSelector
    >
  ...
  describe("Element Selection", () => {
    ...
    it("should deselect previous element when selecting new one", () => {
      ...
      expect(mockStateManager.setSelectedElement).toHaveBeenCalledWith(
        testButton
      )
    })
  ...
  describe("Hover Tooltip Management", () => {
    ...
    it("should remove existing tooltip before showing new one", () => {
      ...
      elementActions.showHoverTooltip(testDiv, 100, 200)
      expect(
        document.querySelectorAll(".absmartly-hover-tooltip")
      ).toHaveLength(1)
  ...
  describe("Element Manipulation - Change Image Source", () => {
    ...
    beforeEach(() => {
      ...
      testDivWithBg.style.backgroundImage =
        "url('https://example.com/old-bg.jpg')"
    ...
    it("should change background-image style", async () => {
      ...
      const mockShow = jest
        .fn()
        .mockResolvedValue("https://example.com/new-bg.jpg")
      ;(elementActions as any).imageSourceDialog.show = mockShow
      ...
      expect(mockUndoRedoManager.addChange).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          type: "style",
          value: {
            "background-image": "url('https://example.com/new-bg.jpg')"
          },
          mode: "merge"
        }),
        null
  ...
  describe("Edge Cases and Error Handling", () => {
    it("should handle element selection with no existing classes", () => {
      ...
      expect(elementWithoutClass.classList.contains("absmartly-selected")).toBe(
        true
      )
    })

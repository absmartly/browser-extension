...
import type { VisualEditorConfig, VisualEditorState } from "../state-manager"
...
describe("StateManager", () => {
    ...
    it("should replace entire state with setState", () => {
      const newState: VisualEditorState = {
        ...
        undoStack: [
          {
            type: "add",
            change: { selector: ".test", type: "text", value: "Test" },
            index: 0
          }
        ],
        redoStack: [
  ...
  describe("Changes Management", () => {
    ...
    it("should replace existing changes when setting", () => {
      ...
      const newChanges: any[] = [
        { selector: ".new", type: "text", value: "New" }
      ]
  ...
  describe("Original Values Management", () => {
    ...
    it("should handle different data types as values", () => {
      ...
      expect(stateManager.getOriginalValue("object-key")).toEqual({
        prop: "value"
      })
  ...
  describe("Complex State Transitions", () => {
    ...
    it("should handle undo/redo workflow", () => {
      ...
      stateManager.pushUndo({
        type: "add",
        change: { selector: ".test1", type: "text", value: "Text1" } as any,
        index: 0
      })
      ...
      stateManager.pushUndo({
        type: "add",
        change: { selector: ".test2", type: "text", value: "Text2" } as any,
        index: 1
      })
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle large undo/redo stacks", () => {
      ...
      for (let i = 0; i < largeStackSize; i++) {
        stateManager.pushUndo({
          type: "add",
          change: { selector: `.test${i}`, type: "text", value: "test" } as any,
          index: i
        })
      }

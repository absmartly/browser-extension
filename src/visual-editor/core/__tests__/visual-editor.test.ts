...
import { initVisualEditor, VisualEditor } from "../visual-editor"
...
const MockEventHandlers = EventHandlers as jest.MockedClass<
...
const MockNotifications = Notifications as jest.MockedClass<
  typeof Notifications
>
const MockElementActions = ElementActions as jest.MockedClass<
  typeof ElementActions
>
const MockEditorCoordinator = EditorCoordinator as jest.MockedClass<
  typeof EditorCoordinator
>
...
describe("VisualEditor", () => {
  ...
  beforeEach(() => {
    ...
    mockStateManager = {
      ...
      getState: jest
        .fn()
        .mockReturnValue({ changes: [], undoStack: [], redoStack: [] }),
  ...
  afterEach(() => {
    ...
    const styles = document.querySelectorAll(
      'style[id="absmartly-visual-editor-styles"], style[data-absmartly="true"]'
    )
  ...
  describe("start()", () => {
    ...
    it("should inject styles when starting", () => {
      ...
      const editorStyles = document.getElementById(
        "absmartly-visual-editor-styles"
      )
      const globalStyles = document.querySelector(
        'style[data-absmartly="true"]'
      )
  ...
  describe("stop()", () => {
    ...
    it("should remove styles when stopping", () => {
      ...
      const removedStyles = document.getElementById(
        "absmartly-visual-editor-styles"
      )
  ...
  describe("Style Management", () => {
    ...
    it("should remove styles when stopped", () => {
      ...
      // Verify styles exist
      expect(
        document.getElementById("absmartly-visual-editor-styles")
      ).not.toBeNull()
  ...
  describe("saveChanges()", () => {
    ...
    it("should save changes and show notification", () => {
      ...
      const callbacks = coordinatorCallArgs[
        coordinatorCallArgs.length - 1
      ] as EditorCoordinatorCallbacks
  ...
  describe("Integration with ElementActions", () => {
    ...
    it("should delegate element operations to ElementActions", () => {
      ...
      editor["undoRedoManager"].addChange(
  ...
  describe("Edge Cases and Error Handling", () => {
    ...
    it("should handle onChangesUpdate callback errors in saveChanges", () => {
      ...
      editorWithError["changes"] = [
        { selector: ".test", type: "text", value: "test" }
      ]

      const coordinatorCallArgs =
        MockEditorCoordinator.mock.calls[
          MockEditorCoordinator.mock.calls.length - 1
        ]
      const callbacks = coordinatorCallArgs[
        coordinatorCallArgs.length - 1
      ] as EditorCoordinatorCallbacks
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        "Error in onChangesUpdate callback:",
        expect.any(Error)
      )
...
describe("initVisualEditor", () => {
  ...
  it("should handle onChangesUpdate callback correctly", () => {
    ...
    editorInstance.getChanges = jest
      .fn()
      .mockReturnValue([
    ...
    const testChanges: DOMChange[] = [
      { selector: ".test", type: "text", value: "Updated content" }
    ]
    callbacks.onChangesUpdate(testChanges)

    const targetOrigin =
      window.location.origin === "null" ? "null" : window.location.origin

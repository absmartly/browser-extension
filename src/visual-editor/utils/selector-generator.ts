...
function isAutoGenerated(str: string): boolean {
  ...
  // Look for 8+ character sequences that appear random
  // This regex finds sequences that have irregular mixed case and numbers
  const randomSequencePattern =
    /[a-z][A-Z0-9]{2,}[a-z][A-Z0-9]|[0-9][a-zA-Z]{2,}[0-9]|[a-z]{2,}[A-Z]{2,}[a-z]|[A-Z][a-z][0-9][a-zA-Z]{4,}/

  // Also check for 6+ character sequences that are all lowercase/uppercase with numbers mixed in
  // but don't look like normal naming (like "item1" or "page2")
  ...
  for (const segment of segments) {
    ...
    const looksLikeWord =
      /^[A-Z][a-z]+$/.test(segment) || // PascalCase word
    ...
    // Check if segment has random characteristics
    ...
    if (segment.length >= 8 && mixedRandomPattern.test(segment)) {
      // Additional check: does it have good case mixing that suggests randomness?
      const hasRandomCaseMixing =
        /[a-z].*[A-Z].*[a-z]/.test(segment) ||
...
function getDataAttributeSelector(element: Element): string | null {
  ...
  if (
    preferred &&
    !isAutoGenerated(preferred.value) &&
    !isDynamicValue(preferred.value)
  ) {
  ...
  const goodAttr = dataAttrs.find(
    (attr) => !isAutoGenerated(attr.value) && !isDynamicValue(attr.value)
  )
  if (goodAttr) {
    // Use CSS.escape to properly escape the value for use in a selector

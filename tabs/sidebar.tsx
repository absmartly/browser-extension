...
try {
  ...
  const logChildren = (label: string) => {
    ...
    console.log(
      `[Sidebar Instrumentation] ${label} children:`,
      el ? el.children.length : "root-missing"
    )
  }
  ...
  const observer = new MutationObserver((mutations) => {
    for (const m of mutations) {
      if (m.type === "childList") {
        ...
        if (!el) {
          console.error(
            "[Sidebar Instrumentation] CRITICAL: __plasmo removed from DOM"
          )
        }
      }
    }
...
const ABSmartlySidebar = () => {
  ...
  useEffect(() => {
      ...
      const observer = new MutationObserver((mutations) => {
        for (const mutation of mutations) {
          ...
          if (
            ...
            const removedPlasmo = Array.from(mutation.removedNodes).some(
              (n) => (n as HTMLElement).id === "__plasmo"
            )
            if (removedPlasmo) {
              console.error(
                "[sidebar.tsx] ⚠️ CRITICAL: __plasmo root was removed from DOM"
              )
            }
          }
        }
    ...
    return () => {
      console.log(
        "[sidebar.tsx] 🔴 ABSmartlySidebar UNMOUNTED",
        mountId.current
      )
    }
  }, [])
  ...
  console.log(
    "[sidebar.tsx] ABSmartlySidebar RENDER, mountId:",
    mountId.current
  )

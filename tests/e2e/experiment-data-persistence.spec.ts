  ...
  test('should persist and reload all experiment metadata including unit type', async () => {
    ...
    await test.step('Select Application', async () => {
      ...
      try {
        ...
        await sidebar.locator('#applications-select-trigger:not([class*="cursor-not-allowed"])').waitFor({ timeout: 60000 })
    ...
    await test.step('Create experiment', async () => {
      ...
      try {
        ...
        await sidebar.locator('#experiments-heading').waitFor({ timeout: 15000 })

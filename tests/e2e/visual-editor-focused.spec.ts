...
test.describe('Focused Visual Editor Test', () => {
  ...
  test.beforeEach(async ({ context, extensionUrl }) => {
    ...
    const { sidebar: _ } = await setupTestPage(testPage, extensionUrl, TEST_PAGE_URL)
  ...
  test('Test key visual editor operations and save', async ({ extensionUrl }) => {
    ...
    log('1️⃣ Edit Text...')
    const title = testPage.locator('#hero-title').first()
